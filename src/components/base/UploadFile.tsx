    ...
    const handleChange: UploadProps<UploadFileResponse>['onChange'] = info => {
        ...
        newFileList = newFileList.map(file => {
            if (file.response) {
                ...
                const url = new URL(file.response.files?.at(0)?.downloadLink ?? "")
        ...
        props.fileListProcessHook((newFileList.map(file => file.response?.files).filter(files => files !== undefined) as FileResponse[][]).flat())
    };
