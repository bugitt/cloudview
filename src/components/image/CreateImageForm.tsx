...
import {FileResponse, PostProjectProjectIdImagesRequest} from "../../cloudapi-client";
import {ProjectIdProps} from "../../assets/types";
import {FileUpload} from "../base/UploadFile";
import {cloudapiClient, getUserId, messageError, messageSuccess} from "../../utils";
...
export const CreateImageForm = (props: ProjectIdProps) => {
    ...
    const onFinish = async (values: CreateImageFormData) => {
        ...
        try {
            await cloudapiClient.postProjectProjectIdImages(props.projectId, req)
    ...
    return (
        ...
        >
            ...
            <Form.Item name="tag" label="镜像标签" rules={[{required: false}]}>
                <Input/>
            </Form.Item>
            <Form.Item label="构建方式" name='buildType'
                ...
                >
                    ...
                    <Select.Option value="localFile">上传本地压缩包</Select.Option>
                    <Select.Option value="localRemoteLink">使用远程压缩包链接</Select.Option>
                    <Select.Option value="raw">直接从单独的Dockerfile构建</Select.Option>
                </Select>
            </Form.Item>
            {/*不同的上下文构建方式*/}
            {
                        ...
                        <>
                            <Form.Item label="Git仓库地址" name="gitUrl"
                                       rules={[
                                           ...
                                           {type: 'url', message: '请输入合法的使用HTTP协议的Git仓库地址'}
                                       ]}
                            >
                                <Input placeholder="请保证以 http:// 或 https:// 开头"/>
                            </Form.Item>
                            <Form.Item label='Git仓库用户名' name='gitUsername'
                                       extra='如果Git仓库为私有仓库，请输入有访问权限的用户名'>
                                <Input/>
                            </Form.Item>
                            <Form.Item label='Git仓库密码' name='gitPassword'
                                       extra='如果Git仓库为私有仓库，请输入有访问权限的用户对应的密码或Token'>
                                <Input/>
                            </Form.Item>
                            <Form.Item label='Git仓库引用' name='gitRef'
                    ...
                    : buildType === 'localFile' ? (
                            <>
                                <Form.Item label="构建上下文压缩包" name="fileList">
                                    <FileUpload fileType={'ImageBuildContextTar'} owner={getUserId()}
                                                involvedId={props.projectId} maxCount={1}
                        ...
                        : buildType === 'localRemoteLink' ? (
                                <>
                                    <Form.Item label="远程压缩包链接" name="contextFileLink"
                                               rules={[
                                                   {required: true, message: '请输入远程压缩包链接'},
                                                   {type: 'url', message: '请输入合法的使用HTTP协议的远程压缩包链接'}
                                               ]}
                                    >
                                        <Input placeholder="请保证以 http:// 或 https:// 开头"/>
                                    </Form.Item>
            ...
            {/*不同的dockerfile选择*/}
            {buildType === 'raw' || buildType === "" ? null :
                <Form.Item label="如何指定Dockerfile" name='dockerfileType'
                           rules={[{required: true, message: '请选择如何指定Dockerfile'}]}>
                    <Select
                        placeholder="请选择如何指定Dockerfile"
                        onChange={onDockerfileTypeChange}
                    >
                        <Select.Option value="dockerfileContent">手动输入Dockerfile内容</Select.Option>
                        <Select.Option value="dockerfilePath">使用上下文中已有的Dockerfile</Select.Option>
                    </Select>
                </Form.Item>
            }
            {
                dockerfileType === 'dockerfileContent' ? (
                    <>
                        <Form.Item label="Dockerfile" name="dockerfileContent"
